...
class _OwnerProfileScreenState extends State<OwnerProfileScreen> {
  ...
  Future<void> _loadUserProfile() async {
    ...
    try {
      ...
      // Load shop details from pasalubong_centers
      ...
      if (shopQuery.docs.isNotEmpty) {
        ...
        setState(() {
          ...
          _shopImageBase64 = shopData['shopImageBase64'];
        });
      }
  ...
  Future<void> _saveProfile() async {
    ...
    try {
      ...
      // Convert shop image to base64 if a new one was selected
      String? shopImageBase64 = _shopImageBase64;
      if (_shopImageFile != null) {
        shopImageBase64 = await _convertImageToBase64(_shopImageFile!);
        if (shopImageBase64 == null && mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text(
                'Shop image processing failed. Profile will be saved without shop photo.',
              ),
              backgroundColor: Colors.orange,
            ),
          );
      ...
      // Update shop details in pasalubong_centers collection
      if (_shopNameController.text.trim().isNotEmpty) {
        ...
        if (shopQuery.docs.isNotEmpty) {
          ...
          await shopQuery.docs.first.reference.update({
            'name': _shopNameController.text.trim(),
            'operatingHours': _operatingHoursController.text.trim(),
            'hours': _operatingHoursController.text.trim(),
            if (shopImageBase64 != null) 'shopImageBase64': shopImageBase64,
            'updatedAt': FieldValue.serverTimestamp(),
          });
        } else {
      ...
      setState(() {
        ...
        _shopImageBase64 = shopImageBase64;
        _imageFile = null;
        _shopImageFile = null;
        _isLoading = false;
        _isEditing = false;
  ...
  Future<void> _pickShopImage() async {
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Error picking shop image: $e'),
            backgroundColor: Colors.red,
          ),
        );
      }
  ...
  Future<void> _selectOperatingHours() async {
    ...
    if (!mounted) return;
    final startFormatted = pickedStartTime.format(context);
    final endFormatted = pickedEndTime.format(context);
    final hoursText = '$startFormatted - $endFormatted';

    setState(() {
      _operatingHoursController.text = hoursText;
    });
  }
                ...
                child: Column(
                  children: [
                    ...
                    TextFormField(
                      ...
                      validator: (value) {
                        if (value == null || value.isEmpty) {
                          return 'Please select operating hours';
                        }
                        return null;
                      },
                    ),
  ...
  String _getInitials() {
    ...
    return initials.isEmpty ? 'O' : initials;
  }
