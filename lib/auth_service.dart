...
class AuthService {
  ...
  void seedDemoAccount() {
    ...
    // Add a default admin account for local testing (in-memory only).
    // Only seed this in debug mode so it won't be present in production.
    if (kDebugMode) {
      _users.putIfAbsent(
        'admin@daho.app',
        () => {'password': 'adminpassword', 'type': 'admin'},
      );
    }
  }
}
